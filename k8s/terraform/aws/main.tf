...
resource "kubernetes_config_map" "grafana_dashboard" {
  ...
  metadata {
    name      = "connectivity-service-dashboard"
  ...
  data = {
    "connectivity-service-dashboard.json" = file("${path.module}/../../../grafana/provisioning/dashboards/connectivity-service-dashboard.json")
  }
}
...
resource "kubernetes_manifest" "prometheus_rule" {
  ...
  manifest = {
    ...
    kind       = "PrometheusRule"
    metadata = {
      name      = "connectivity-service-alerts"
      namespace = "monitoring"
      labels = {
